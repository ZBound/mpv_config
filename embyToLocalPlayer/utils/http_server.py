...
def start_play(data):
    ...
    global_time_diff = start_time - global_start_time  # 解析数据和重定向的时间，约等于是重定向耗时。
    ...
    if player_name:
        ...
        if configs.check_str_match(_str=data['netloc'], section='playlist', option='enable_host') \
            ...
            if is_multiple_episodes:
                ...
                main_ep_info = data.get('main_ep_info', {})
                main_ep_info['index'] = main_ep_info.get('IndexNumber')
                data.update(main_ep_info)  # trakt bgm sync 需要 ProviderIds 等数据。
                eps_data = [data]  # 追更时，只有一集，不需要等待线程。
            not is_multiple_episodes and logger.info(f'{is_multiple_episodes=}, skip eps_data wait.')
            _t_eps_data = time.time() - _t

            _t = time.time()
            player_manager.playlist_add(eps_data=eps_data)
            _t_playlist = time.time() - _t

            logger.info('player started.')
            logger.info(f'parse and redirect time={str(global_time_diff)[:5]}')
            logger.info(f'init_player wait time={str(_t_init_player)[:5]}')
            logger.info(f'eps_data wait time={str(_t_eps_data)[:5]}')
            logger.info(f'playlist_add wait time={str(_t_playlist)[:5]}')
            logger.info(f'all used time={str(time.time() - global_start_time)[:5]}')
