...
class PlayerManager:
    ...
    def playlist_add(self, eps_data=None):
        ...
        # 因为改版脚本缺集时不禁用播放列表，可能导致mpv的下一集和emby下一集对不上，先不启用，可测试没问题后启用。
...
def mpv_player_start(cmd, start_sec=None, sub_file=None, media_title=None, get_stop_sec=True, mount_disk_mode=None,
    ...
    if not mount_disk_mode:
        if proxy := configs.player_proxy:
            ...
            is_multiple_episodes and cmd.append('--cache=no')
    ...
    if configs.disable_audio:
        cmd.append('--no-audio')
        # cmd.append('--no-video')
    is_multiple_episodes and cmd.append(f'--pause')
...
def playlist_add_mpv(mpv: MPV, data, eps_data=None, limit=10):
    ...
    for ep in episodes:
        ...
        if basename == data['basename']:
            # 元数据请求不包含episodesInfo时会导致title_intro_map_fail，
            # 进而会设置ep['media_title'] = data['media_title']，故同步上游。
            append = True
            continue
        # iina 添加不上
    ...
    while True:
        try:
            ...
            if not tmp_sec:
                ...
                if start_time:
                    logger.info(f'mpv finish init used time={str(time.time() - start_time)[:5]}')
                    start_time = None
                mpv_play_speed[media_title] = speed
                stop_sec = tmp_sec
