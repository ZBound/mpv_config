...
class NoRedirectFor302(urllib.request.HTTPRedirectHandler):
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        if code == 302:
            import urllib.error
            raise urllib.error.HTTPError(req.full_url, code, msg, headers, fp)
        super().redirect_request(req, fp, code, msg, headers, newurl)


# 因为改版脚本缺集时不禁用播放列表，可能导致mpv的下一集和emby下一集对不上。
# 所以用不上重定向下一集，里面的跟随302功能（适配多次跳转，但可能慢），因此暂不同步上游。
def get_redirect_url(url, key_trim='PlaySessionId'):
    ...
    start = time.time()
    try:
        req = requests_urllib(url, req_only=True)
        opener = urllib.request.build_opener(NoRedirectFor302())
        jump_url = opener.open(req, timeout=3).url
...
def list_episodes(data: dict):
    ...
    def version_filter(file_path, episodes_data):
        ...
        ini_re = [re.escape(component) for component in ini_re]
        logger.info(f'version_filter: {ini_re=}')
    ...
    logger.info(
        f'<title_intro_index_map>: {title_data}')  # 有时episodes_info有可能为空，此时title_data也为空，因此会导致下方parse_item函数将media_title设置为仅basename
